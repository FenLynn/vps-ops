...
### 7. Ubuntu 24.04 的 `ssh.socket` 接管：改了端口却永远进不去（已修复）
**症状**：`init_host.sh` 跑完，日志显示 SSH 正常重启，阿里云安全组已放行 22222，但连接时报 `Remote side unexpectedly closed network connection`——端口是通的，却瞬间断开。
**血泪**：Ubuntu 22.10+ 引入了 **systemd socket activation**，`ssh.socket` 单元接管了 22 端口监听权。你无论如何修改 `sshd_config`、无论重启多少次 `ssh.service`，22222 端口都不会被监听，因为 `systemd` 这个"房东"从来没有被通知要换门。`ss -tulpn | grep 22222` 永远是空的。
**破局**：在 SSH 配置写入后、重启前显式禁用 `ssh.socket`，将端口控制权归还给 `sshd`：
```bash
systemctl disable --now ssh.socket
systemctl enable ssh.service
systemctl restart ssh
```
`init_host.sh` 现已通过 `disable_ssh_socket_if_needed()` 函数自动处理此逻辑，并在重启后验证端口是否真正监听。
...
**核心陷阱：** Ubuntu 22.10+ 引入了 `ssh.socket` 接管端口监听权，导致 `sshd_config` 修改的端口配置失效。`init_host.sh` 已自动处理，但如果你想手动确认或重建SSH配置：
...
# ❗ Ubuntu 24.04 必须：禁用 ssh.socket，否则自定义端口永远无效
systemctl disable --now ssh.socket
systemctl enable ssh.service
systemctl restart ssh
...
```bash
# 从任一已安装 Tailscale 的设备，无需私鑰
# Tailscale 设备名查看： tailscale status
...
| 维度 | 传统 SSH | Tailscale SSH |
|---|---|---|
| 公网暴露 | 开放 22222 端口 | 公网零端口暴露 |
| 信任基础 | 私鑰文件 | GitHub/Google SSO 身份 |
| 就迷时 | 没带私鑰=进不去 | 探头登录 Tailscale 即可 |
| 全网扫描 | 面对全网扫描 | 免疫 |

> ❗ **建议保留传统 SSH 22222 作为应急备用通道**，Tailscale 值网器故障时可用 VNC 作最后随手。

---

## 🌐 高级安全（可选）：Cloudflare Browser SSH — 纲页端性个人终端

> 通过浏览器直接 SSH，无需安装任何客户端。  
> **适合场景：** 借用别人的电脑、免版 iPad 等轻量场景。

### 配置步骤（纯 CF Dashboard 操作无需改代码）

**Step 1：在 CF Tunnel 配置中新增 SSH 路由**

登录 [Cloudflare Zero Trust](https://one.dash.cloudflare.com/) → `Networks` → `Tunnels` →选中当前 Tunnel → `Edit` → `Public Hostname` → **Add a public hostname**：
- Subdomain: `ssh`
- Domain: `660415.xyz`
- Service Type: `SSH`
- URL: `localhost:22222`

**Step 2：创建 Access 应用保护它**

`Access` → `Applications` → `Add an application` → `Self-hosted`：
- Application Name: `VPS SSH`
- Application Domain: `ssh.660415.xyz`
- Policy: 含你 GitHub 邮筱的 Allow 规则

**Step 3：在 VPS 上生成 SSH Host 证书（让浏览器能验证服务器身份）**

```bash
# 在 VPS 上执行
cloudflared access ssh-config --hostname ssh.660415.xyz --short-lived-cert
# 按提示将输出内容填入 ~/.ssh/config （本地设备操作）
```

**Step 4：使用**

浏览器打开 `https://ssh.660415.xyz` → CF Access 弹出验证 → 网页终端即弹出。
...
# 步骤 3: 重启 derper 读取新证书
docker compose restart derper

# 验证 derper 证书加载成功
docker logs derper --tail 5
# 期望看到: derper: serving on :33445 with TLS
```

> ⚠️ **注意**：LE 对同一域名每周限 5 次正式证书签发。演练阶段请务必在 `.env` 设置 `ACME_STAGING=true`，彩排完全定型后再切换正式版！
